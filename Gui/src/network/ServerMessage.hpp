/**
 * @file network/ServerMessage.hpp
 * @brief Typed structs for every server-to-GUI protocol message, plus the ServerMessage variant.
 * @details Responsibility: define one plain-data struct per message type described in the
 *          zappy GUI protocol specification. No parsing logic lives here — this file is
 *          pure data definitions.
 *
 *          Architecture: NetworkClient::parseLine() constructs one of these structs and
 *          wraps it in a ServerMessage variant value. The main thread receives that value
 ...
 * @details Protocol line: "tna TeamName"
 *          The server sends one line per team.
 */
struct MsgTeamName {
 ...
 * @details Protocol line: "sst t"
 *          Sent by the server after the GUI calls "sst <t>".
 */
 ...
 * @details Protocol line: "smg message text"
 *          Free-form informational string from the server.
 */
 ...
 * @details Every alternative corresponds to exactly one protocol command.
 *
 *          Use std::visit to dispatch on the active type:
 *          @code
 *          std::visit([](auto&& msg) { handle(msg); }, serverMessage);
 *          @endcode
 *
 */
